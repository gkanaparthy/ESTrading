...
namespace NinjaTrader.NinjaScript.Strategies
{
    ...
    // ✅ NEW: Order cancellation state tracking
    private bool pendingOrderCancellation = false;
    ...
    pendingOrderCancellation = false;
    ...
    SystemPerformance.AllTrades.TradesPerformance.Currency.CumProfit - PrevDayPnL;
			    ...
			    double profitTarget = 1000 * TradeSize;
    ...
    // ✅ 3) FIXED Entry logic with proper order cancellation sequencing
    ...
    // Determine current preferred direction
    bool preferLong = (Close[0] - atrLowband) < (atrHighband - Close[0]);
    bool preferShort = (Close[0] - atrLowband) > (atrHighband - Close[0]);

    // Check if we need to cancel existing opposing order
    bool needsCancellation = false;
    if (atrEntryOrder != null && !pendingOrderCancellation)
    {
        bool isLongOrder = atrEntryOrder.Name.StartsWith("ATR Long");
        bool isShortOrder = atrEntryOrder.Name.StartsWith("ATR Short");

        // Cancel if order direction doesn't match current preference
        if ((isLongOrder && preferShort) || (isShortOrder && preferLong))
        {
            CancelOrder(atrEntryOrder);
            pendingOrderCancellation = true; // ✅ Track cancellation state
            Print("[DEBUG] Cancelled opposing order: " + atrEntryOrder.Name + " - switching direction");
            needsCancellation = true;
        }
    }

    // Only proceed with new orders if no active order and no pending cancellation
    if (atrEntryOrder == null && !pendingOrderCancellation && !needsCancellation)
    {
        bool atrLevelRevisitedWithSufficientRetracement = false;
        double currentATR = ATR(Closes[1], 14)[0];
        if (lastATRLevelTouchBarIndex.TryGetValue(atrCurrRecordedLevel, out int lastTouch))
        {
            ...
            && (Time[0] - lastOrderCloseTime).TotalMinutes >= 5)
        {
            ...
            if (Math.Abs(vwapValue - atrHighband) <= 3 && vwapValue > atrHighband)
                atrAdjEntry = vwapValue;
            ...
            SetProfitTarget("ATR Short", CalculationMode.Ticks, Math.Min(4 * 20, 4 * (atrAdjEntry - atrLowband)));
            BE_Set = Trail_Set = false;
            currentTrailStopPrice = 0;
            Print("[DEBUG] SHORT ENTRY SIGNAL: Price=" + atrAdjEntry + ", SL=" + (atrHighband + 4));
        }
        // ATR LONG ENTRY
        else if (preferLong
                 && (atrPrevTouchedLevel == double.MinValue ||
                 ...
                 && (Time[0] - lastOrderCloseTime).TotalMinutes >= 5)
        {
            ...
            if (Math.Abs(vwapValue - atrLowband) <= 3 && vwapValue < atrLowband)
                atrAdjEntry = vwapValue;
            ...
            SetProfitTarget("ATR Long", CalculationMode.Ticks, Math.Min(4 * 20, 4 * (atrHighband - atrAdjEntry)));
            BE_Set = Trail_Set = false;
            currentTrailStopPrice = 0;
            Print("[DEBUG] LONG ENTRY SIGNAL: Price=" + atrAdjEntry + ", SL=" + (atrLowband - 4));
        }
    }
    else if (pendingOrderCancellation)
    {
        Print("[DEBUG] Skipping entry - pending order cancellation");
    }
    }
    }
    ...
    {
    if (Close[0] >= Instrument.MasterInstrument.
    RoundToTickSize(Position.AveragePrice +
    ATR(Closes[1], 14)[0]) &&
    !BE_Set)
    {
    SetStopLoss(CalculationMode.Price, Position.AveragePrice);
    BE_Set = true;
    Print("[DEBUG] Long moved to breakeven at " +
    ...
    double minutesSinceEntry =
    (Time[0] - orderTime).TotalMinutes - BarsPeriod.Value;
    ...
    {
    ExitLong();
    Print($"[DEBUG] Exiting Long → Close below AVWAP after " +
    $"{minutesSinceEntry:F2} mins: AVWAP = " +
    anchoredVWAP[0]);
    anchorBar = -1;
    cumPV = cumVol = 0;
    anchoredVWAP[0] = double.NaN;
    ...
    Close[2] < Close[3] - 0.5 &&
    ((Time[0] - orderTime).TotalMinutes -
    BarsPeriod.Value) > 4)
    {
    ExitLong();
    Print($"[DEBUG] Exiting Long → Bearish PA exit at " +
    $"{((Time[0] - orderTime).TotalMinutes - BarsPeriod.Value):F2} mins");
    anchorBar = -1;
    cumPV = cumVol = 0;
    anchoredVWAP[0] = double.NaN;
    }
    ...
    SetStopLoss(CalculationMode.Price, Position.AveragePrice);
    BE_Set = true;
    Print("[DEBUG] Short moved to breakeven at " +
    Position.AveragePrice);
    }
    ...
    {
    ExitShort();
    Print($"[DEBUG] Exiting Short → Close above AVWAP " +
    $"after {minutesSinceEntry:F2} mins: AVWAP = " +
    anchoredVWAP[0]);
    anchorBar = -1;
    cumPV = cumVol = 0;
    anchoredVWAP[0] = double.NaN;
    ...
    {
    ExitShort();
    Print($"[DEBUG] Exiting Short → Bullish PA exit at " +
    $"{((Time[0] - orderTime).TotalMinutes - BarsPeriod.Value):F2} mins");
    anchorBar = -1;
    cumPV = cumVol = 0;
    anchoredVWAP[0] = double.NaN;
    ...
    // ✅ FIXED OnOrderUpdate with proper state management
    ...
    if (atrEntryOrder != null && (order.OrderState == OrderState.Cancelled || order.OrderState == OrderState.Rejected))
    {
        Print("[DEBUG] Order " + order.OrderState + ": " + order.Name);
        atrEntryOrder = null;
        pendingOrderCancellation = false; // ✅ Clear cancellation flag ONLY after confirmation
    }
    }
    ...
    atrOrderDilowband = atrLowband;
    atrOrderDihighband = atrHighband;
    atrOrderDinextband = execution.Order.Name.StartsWith("ATR Long")
    ? atrNextbandL : atrNextbandS;

    Print($"[DEBUG] ORDER FILLED: {execution.Order.Name} at " +
    $"{price}, Position={Position.Quantity}");
    }

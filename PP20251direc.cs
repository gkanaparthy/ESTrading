...
{
    public class PP20251direc : Strategy
    {
  ...
  Print("START: "+Time[0]);
   NowPivot=double.NaN;
	...
	if (!double.IsNaN(NowPivot) && NowPivot!= currRecordedPivot )
	{
		prevTouchedPivot=currRecordedPivot;
		currRecordedPivot=NowPivot;
	}   
    
    Print("prevTouchedPivot: " + prevTouchedPivot + "  currRecordedPivot: " + currRecordedPivot + "   NowPivot: "+NowPivot);
    ...
    if (Position.MarketPosition == MarketPosition.Flat || Position.Quantity == Convert.ToInt32(TradeSize))
    {
      ...
      //  vwapShortFlag = entryOrder == null && Low[2] < vwPrice && High[2] > vwPrice && High[1] < vwPrice;
        ...
        if (entryOrder == null //&& newPivotTouched 
            ...
            currRecordedPivot < prevTouchedPivot && ATR(Closes[1], 14)[0] < 90)
        {
            Print("SHORT ENTRY: currRecordedPivot (" + currRecordedPivot + ") < prevTouchedPivot (" + prevTouchedPivot + ")");
			adjEntry=Open[0];
			if ((highband + 0.5 - Open[0])> 5)
				adjEntry=highband-5;
            ...
            SetStopLoss(CalculationMode.Price, highband + 0.5);
            SetProfitTarget("Short VWAP", CalculationMode.Ticks, 4*20);
            BE_Set = false;
            Print("Entering Short VWAP: StopLoss at " + (highband + 0.5) + ", ProfitTarget at " + (adjEntry-20));
			...
			&& (lowbandLongFlag || highbandLongFlag || vwapLongFlag) && 
                currRecordedPivot > prevTouchedPivot && ATR(Closes[1], 14)[0] < 90)
        {
            Print("LONG ENTRY: currRecordedPivot (" + currRecordedPivot + ") > prevTouchedPivot (" + prevTouchedPivot + ")");
			adjEntry=Open[0];
			if ((Open[0] - (lowband - 0.5))>5)
				adjEntry=lowband+5;
            EnterLongLimit(0, false, Convert.ToInt32(TradeSize), adjEntry, "Long VWAP");
            OrderDilowband = lowband;
            OrderDihighband = highband;
            OrderDinextband = nextbandL;
            SetStopLoss(CalculationMode.Price, lowband - 0.5);
            SetProfitTarget("Long VWAP", CalculationMode.Ticks,  4*20);
            BE_Set = false;
            Print("Entering Long VWAP: StopLoss at " + (lowband - 0.5) + ", ProfitTarget at " + (adjEntry+20));

    ...
    public class ATRQAVWAPTrail : Strategy
    {
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (SystemPerformance.AllTrades.Count > 0 && !dayOverVar)
            {
               ...
               // if (AccountRealizedPL < 400//MaxLoss* TradeSize
				//	||
                  //  AccountRealizedPL > 300 * TradeSize ||
...
        	double VWAPValue = VWAP1(BarsArray[0], new VWAPDesign.StdDesign { Enabled = false, Num = 1 }, new VWAPDesign.StdDesign { Enabled = false, Num = 2 }, new VWAPDesign.StdDesign { Enabled = false, Num = 3 }, true, true, true).Output[0];
	 ...
	         vwapValue = Instrument.MasterInstrument.RoundToTickSize(VWAPValue);
                ...
                if (Position.MarketPosition == MarketPosition.Flat && atrLevels != null)
                {
                    ...
                    {
                        Print($"[DEBUG] Price {Close[0]:F2} outside ±2ATR bounds " +
                              $"[{minATR:F2}–{maxATR:F2}]; skipping entry");
                    }
                    else
                    {
                        ...
                        SetATRBands(Close[0]);
                        atrLowbandLongFlag = atrEntryOrder == null &&
                                            (Close[0] - atrLowband) < (atrHighband - Close[0]);
                        atrHighbandShortFlag = atrEntryOrder == null &&
                                              (Close[0] - atrLowband) > (atrHighband - Close[0]);
                                ...
                                atrCurrRecordedLevel, out int lastTouch))
                        {
                            ...
                            atrLevelRevisitedWithSufficientRetracement =
                                atrDeviation > 2 * currentATR;
                            if (atrLevelRevisitedWithSufficientRetracement)
                                Print("[DEBUG] ATR Level " + atrCurrRecordedLevel +
                        ...
                        if (atrEntryOrder == null && atrHighbandShortFlag
                            && (atrPrevTouchedLevel == double.MinValue ||
                                ...
                                atrLevelRevisitedWithSufficientRetracement)
                            && ATR(Closes[1], 14)[0] < 6 && (Time[0] - lastOrderCloseTime).TotalMinutes >= 5)
                        {
                            ...
                            Print("[DEBUG] SHORT ENTRY SIGNAL: Price=" + atrAdjEntry +
                                  ", SL=" + (atrHighband + 4));
                        }
                        // ATR LONG ENTRY
                                 ...
                                 && ATR(Closes[1], 14)[0] < 6 && (Time[0] - lastOrderCloseTime).TotalMinutes >= 5)
                        {
                            ...
                            BE_Set = Trail_Set = false;
                            currentTrailStopPrice = 0;
                            Print("[DEBUG] LONG ENTRY SIGNAL: Price=" + atrAdjEntry +
                                  ", SL=" + (atrLowband - 4));
                        }
                    }
                }
        ...
        {
            if (atrEntryOrder != null && atrEntryOrder.IsBacktestOrder &&
                State == State.Realtime)
                atrEntryOrder = GetRealtimeOrder(atrEntryOrder);

            if (atrEntryOrder == null &&
                (order.Name.StartsWith("ATR Long") ||
                 order.Name.StartsWith("ATR Short")))
            {
                atrEntryOrder = order;
                Print("[DEBUG] Order placed: " + order.Name +
                      " at " + limitPrice);
            }
    ...
    if (Position.MarketPosition == MarketPosition.Flat)
    {
        ...
        {
           /* AccountRealizedPL = account.Get(AccountItem.RealizedProfitLoss,
                Currency.UsDollar);
            AccountUnrealizedPL = account.Get(AccountItem.UnrealizedProfitLoss,
                Currency.UsDollar);*/

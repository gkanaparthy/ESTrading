...
{
    public class PP202051direcB4logs : Strategy
    {
        ...
        private double lowband, highband, nextbandL, nextbandS;
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (SystemPerformance.AllTrades.Count > 0 && !dayOverVar)
            {
                ...
                if (cumProfit < MaxLoss || totalPL < MaxLoss || totalPL > 500 * TradeSize || AccountUnrealizedPL < -500)
                {
                    dayOverVar = true;
            ...
            // Main trading logic
            if (BarsInProgress == 0 && IsFirstTickOfBar && isitRTH && !isitEarly && toTime > 825 && !dayOverVar)
            {
                prevLow = PriorDayOHLC().PriorLow[0];
                prevHigh = PriorDayOHLC().PriorHigh[0];
                ...
                if (currentBandIndex != prevBandIndex && prevBandIndex != -1)
                {
                    prevTouchedPivot = lastTouchedPivot;
                    lastTouchedPivot = currentBandIndex > prevBandIndex ? levels[currentBandIndex] : levels[currentBandIndex + 1];
                }
                ...
                if (Position.MarketPosition == MarketPosition.Flat || Position.Quantity == Convert.ToInt32(TradeSize))
                {
                    ...
                    // Enter trades with pivot direction check
                    if (entryOrder == null && (lowbandShortFlag || highbandShortFlag || vwapShortFlag) && !double.IsNaN(prevTouchedPivot) && prevTouchedPivot > Close[0])
                    {
                        ...
                        SetProfitTarget("Short VWAP", CalculationMode.Ticks, 8 * (highband + 0.25 - Close[0]));
                        BE_Set = false;
                    }
                    else if (entryOrder == null && (lowbandLongFlag || highbandLongFlag || vwapLongFlag) && !double.IsNaN(prevTouchedPivot) && prevTouchedPivot < Close[0])
                    {
                        ...
                        SetProfitTarget("Long VWAP", CalculationMode.Ticks, 8 * (Close[0] - (lowband - 0.25)));
                        BE_Set = false;
                    }
                }
        ...
        [Range(1, double.MaxValue), NinjaScriptProperty]
        [Display(ResourceType = typeof(Custom.Resource), Name = "Previous day High", GroupName = "Parameters", Order = 3)]
        public double ManualHighPrice { get; set; }
		
		[Range(1, double.MaxValue), NinjaScriptProperty]
        [Display(ResourceType = typeof(Custom.Resource), Name = "Previous day Settlement", GroupName = "Parameters", Order = 1)]
